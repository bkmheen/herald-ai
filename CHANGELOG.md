...
전면 점검에서 **패키지 안에 박혀 있던 개인 볼트 구성**이 드러났다. IP·자격증명·호스트 이름은
`0.2.47`~`0.2.50` 으로 이미 0건이었으나, **경로와 도구 이름은 남아 있었다.**

### Changed
- **`trip-ledger` 가 개인 볼트 경로를 설정에서 읽는다.** 여태 스킬 문서가 특정인의 볼트
  디렉토리 이름과 미러 스크립트 이름을 **그대로 실행하라고** 적고 있었다
  (`ls -ld ~/<개인 볼트>` 등). 다른 사람이 clone 하면 **존재하지 않는 경로를 본다.**
  `CLAUDE.md` 가 금지한 "스크립트에 내 환경 값을 기본값으로 박지 않는다" 에 정확히 해당한다.

  ```
  VAULT_MIRROR_ROOT   미러 원본 판정 대상 (실디렉토리=원본 / 심볼릭 링크=수신본)
  VAULT_ROOT          노트 볼트 루트 — 이 아래에 Dossier/ 트리가 있다
  VAULT_MIRROR_CMD    미러 실행 명령
  ```

  세 항목 모두 `~/.herald/vault.conf` 에서 읽고, **저장소에는 기본값을 두지 않는다.**
  값이 비어 있으면 볼트 단계를 **수행하지 말고 설정이 필요하다고 보고**한다 — 추정하지 않는다.
  표본은 [`config/vault.conf.example`](config/vault.conf.example) 에 주석으로 넣었다.

- **개발기록의 개인 도구 이름을 일반 서술로.** 미러 스크립트·노트 가공 도구의 실제 이름은
  구성이지 설계가 아니다. 서술의 요지(단방향 미러 · `--delete` · 역방향 채널 없음)는
  이름 없이도 그대로 전달된다.

### 유지
- **`Atelier`·`Dossier` 는 그대로 둔다** (사용자 판단). 이것들은 herald-ai 가 **읽어 오는
  대상**이라, 정할 것은 이름을 지울지가 아니라 **어떻게 읽을지**다. `bin/herald-index` 의
  외부 프론트매터 인식과 `task-tracker` 의 서브커맨드는 **실제로 동작하는 코드**이므로,
  이름을 바꾸면 기존 설치본과 호환이 깨진다.

### 점검 결과 (트리·전 이력 모두 0건)
IP·사설 대역 · `.local` 머신 이메일 · 토큰/시크릿 대입 · SSH 개인키 · 봇 토큰 ·
실호스트 이름. `/home/you/…` 와 `install.sh` 의 주석 구분선은 정규식 오탐이었다.

## [0.2.50] - 2026-08-16

> **묶음 5 — 교차 검토 후 정리 (이어짐)** (slug: `audit`)

### Changed
- **`0.2.49` 가 지운 값을 `0.2.48` 커밋의 파일 내용에서도 걷어냈다.** 파일에서 지우는 것과
  **이력에서 지우는 것은 다른 일이다** — `0.2.49` 는 HEAD 만 고쳤을 뿐, 그 값을 처음 적은
  `0.2.48` 커밋의 blob 에는 그대로 남아 있었다. push 전이라 재작성으로 마저 걷어냈다.
  그래서 `0.2.49` 의 diff 에는 그 교체가 보이지 않는다 — **재작성이 두 커밋을 같은 값으로
  맞췄기 때문**이고, 기록으로서의 `0.2.49` 항목은 그대로 유효하다.

  > **"HEAD 에서 지웠다" 는 "공개되지 않는다" 가 아니다.** 이미 커밋된 값은 재작성으로만
  > 사라진다. push 하기 전이라면 아직 늦지 않았다.

## [0.2.49] - 2026-08-16

### Fixed
- **`0.2.48` 의 DEVLOG 예시가 실제 맥 이름을 되살렸다.** 재작성으로 그 이름을 지운 직후,
  **그것을 지웠다고 설명하는 문장에서** 검색어로 그대로 적었다. 바로 앞 항목이 "원칙은 한 번
  쓴다고 지켜지지 않는다" 고 적어 둔 그 실수를, 같은 커밋 안에서 저지른 셈이다.
  자리표시자로 바꿨다.

  > **예시에 실제 값을 쓰지 않는다** — 값이 무엇인지는 설명에 필요하지 않다.
  > 커밋 직전 전 이력 검사가 이것을 잡았다. 그래서 검사는 push 전이 아니라 **커밋 전**에 돈다.

## [0.2.48] - 2026-08-16

### Changed
- **전 이력을 다시 썼다 — 이번엔 작성자 신원까지** (사용자 지시·force push 승인).
  `0.2.38` 의 재작성은 **파일 내용**과 **커밋 메시지**를 치환했지만, 커밋 6건의 작성자·커밋터
  이메일이 머신 기본값(`<계정>@<호스트>.local`)이라 **개인 계정명과 실제 맥 이름이 그대로
  공개돼 있었다.** 신원은 별개 메타데이터라 어느 치환에도 걸리지 않는다 — `--mailmap` 이
  따로 있다. 44개 커밋 전부가 이제 하나의 공개 주소를 쓴다.
  `0.2.47` 에서 HEAD 에 적용한 식별자 제거도 **같은 재작성으로 과거 커밋까지** 소급했다.

  > **이력 재작성 점검표는 네 곳이다** — 파일 내용 · 커밋 메시지 · **작성자/커밋터 신원** ·
  > 태그 메시지. 하나를 빼면 "다 지웠다" 는 착각만 남는다.

- **`milestone/numbering` 태그 메시지를 작업 구간 기준으로 고쳤다.** 손댄 범위(전 이력)와
  작업이 이뤄진 구간(`0.2.37`~`0.2.43`)을 구분해 적었다. 태그가 서로 겹치지 않아야
  태그로 주제를 가를 수 있다.

### Fixed
- **검증 도구 자체의 거짓 음성을 발견해 검사 방식을 바꿨다.** `git grep <패턴> $ALL --` 처럼
  **변수를 거쳐** 리비전 목록을 넘기면 이 환경에서 git 이 조용히 **0건**을 반환한다.
  HEAD 에 버젓이 있는 문자열도 "없음" 으로 나온다. 이후 전 이력 검사는 인라인 치환
  (`$(git rev-list --all)`)으로 돌리고, **반드시 검출되는 대조군을 함께** 돌려 스캔의 생존을
  확인한다. `0.2.39` 의 "전 이력 재검사 결과 검출 없음" 도 이렇게 얻은 거짓 음성이었을
  가능성이 높다. **"검출 0건" 과 "스캔이 죽었다" 는 겉보기가 같다.**

### Verified
- 전 이력 44개 커밋: 작성자·커밋터 이메일 **단일 공개 주소 1종**, 옛 신원 0건
- 전 이력 파일·커밋 메시지: 실제 맥 이름 · IP 조각 · 사용 금액 · 내부 시스템 이름 **모두 0건**
  (대조군 동시 검사로 스캔 생존 확인)
- 완전한 IP 형태(`n.n.n.n`) 0건
- 릴리스 커밋 38건이 `0.2.10`~`0.2.48` 로 **역행·중복 없이** CHANGELOG 항목과 1:1 대응
- `git-filter-repo` 가 지우는 **리모트와 브랜치 추적을 함께 복구**했다 (`0.2.41` 의 함정)

## [0.2.47] - 2026-08-16

`0.2.38`·`0.2.39` 에서 세운 **「도구는 공개, 구성은 비공개」** 원칙을, 그 뒤에 다시 새어 들어온
값들에 적용한다. 자격증명·개인키 노출은 없었다 — 새어 든 것은 **식별자**다.

### Removed
- **실제 맥 이름 2종.** `0.2.42` 항목이 `herald-host` 실증을 적으면서 이 맥의 이름을 그대로
  옮겨 적었다. 사용자 실명이 포함된 값이다. 기술적 주장("이름 2종이 `since` 와 함께 더해졌다")은
  그대로 두고 값만 걷어냈다. **원칙을 세운 작업(`0.2.38`~`0.2.39`) 바로 다음 묶음에서 재유입된
  것**이라, 원칙은 한 번 쓰는 것으로 지켜지지 않는다는 증거다.
- **사설 IP 를 복원할 수 있는 조각.** 완전한 주소는 어디에도 없었으나 **대역 표기**와
  **끝옥텟 예시**(`@`뒤 세 자리)가 서로 다른 파일에 남아, 둘을 합치면 실주소가 나왔다.
  옥텟 예시는 README 가 이미 쓰던 자리표시자로 통일하고, 대역 표기는 값 없는 서술로 바꿨다.
  **조각도 개인정보다** — 한 저장소 안에 흩어져 있으면 합쳐진다.
- **실제 월 지출액과 머신 누적 토큰량.** `CLAUDE.md` 가 "사용 금액" 을 공개 대상에서 명시적으로
  제외한다. 검증 기록의 요지(`$0.0` 이 실제 값으로 정상화됐다)는 값 없이도 전달된다.
- **내부 시스템 이름·망 구성.** `0.2.24` 항목이 내부 시스템 이름·외부 DB 접속 경로·VPN
  프로토콜을 담고 있었다. 판단의 뼈대만 남기고 구체값은 vault 로 넘겼다.
- **VM 호스트 이름·옛 호스트 이름.** FAQ 와 실증 기록의 실제 이름을 일반 서술로 바꿨다.
  재현에 필요한 것은 "Parallels Win11 게스트" 라는 조건이지 그 VM 의 이름이 아니다.

### Added
- **`.gitignore` 가 도구가 실제로 쓰는 구성·자격증명 파일을 막는다.** 여태 `telegram.conf`
  계열만 막고 있어, `config/vault.conf`(`DROP_HOST`·`DROP_KEY`·`VAULT_TOKEN_FILE`) ·
  `skills/trip-ledger/registry.yaml`(시트 ID) · `id_herald_*`(SSH 개인키) 가 **전부 커밋
  가능한 상태**였다. 사고는 없었으나 구조적으로 열려 있었다 — `config/vault.conf.example`
  이 스스로 복사를 안내하므로, 복사본이 관행적인 이름으로 놓이면 그대로 잡혔다.
  `.claude/settings.local.json` 도 함께 막는다(여태 **이 맥의 전역** 무시 파일로만 걸려 있어,
  다른 맥에서 clone 하면 무시되지 않았다 — `CLAUDE.md` 가 말하는 "규칙은 저장소 안에" 위반).
...
### Verified
- **`herald-host` 연동을 실제 vault 로 실증.** vault 설치 스크립트
  (`bootstrap/herald-vault-setup.sh`)의 5단계가 자동으로 돌아 대장이 관측값을 더했다 —
  이 맥의 이름 **2종**(운영체제가 보고하는 짧은 이름과 사용자가 붙인 표시 이름)과
...
- vault 설치 스크립트(`bootstrap/herald-vault-setup.sh`)의 5단계가 `herald-host observe` 를
  자동으로 돌려 **대장이 저절로 자란다.** (패키지 설치용 `install.sh` 가 아니다)

### 실측
- 옛 이름·옛 `machine_id`·옛 IP·새 `machine_id` **네 가지 모두** 같은 호스트로 해석
...
### 마무리
- 1차 치환이 정확한 주소만 잡아 **대역 표기**(앞 세 옥텟만 적은 형태)를 놓쳐, 2차 패스로
  마저 걷어냈다.

  > ⓘ 당시 이 항목은 "전 이력 재검사 결과 검출 없음" 이라고 적었으나, **그 문장 자신이
  > 검출 대상인 대역 문자열을 그대로 품고 있었다.** 쓰인 시점에 이미 거짓이었다.
  > `0.2.47` 에서 문자열을 없애고 서술을 고쳤다.
...
### Changed
- **일반 호스트가 VPN 에 물려 있는 사유를 기록.** vault 전송과는 무관한 별개 사유이며,
  vault 전송은 사내망 직결로 충분하다고 판정했다. 구성을 바꿔야 할 때가 오면 부분 수정이
  아니라 전체를 한꺼번에 손본다.

  > ⓘ 당시 이 항목은 내부 시스템 이름·외부 DB 접속 경로·VPN 프로토콜을 그대로 적었다.
  > **「도구는 공개, 구성은 비공개」**(`0.2.39`) 판정표에 따르면 전부 vault 쪽이므로,
  > `0.2.47` 에서 구체값을 걷어내고 판단의 뼈대만 남겼다. 구체적인 망 구성은 vault 에 있다.
...
- **README FAQ: VM 게스트 Windows 에서 WSL 불가 시 우회로.** Parallels 등 VM 안 Windows(작업
  ...
  실측: Parallels Win11 게스트 (2026-08-05).

## [0.2.21] - 2026-08-02

### Changed
- **앤솔로지 영역 부여 = 실측으로 「지원됨」 확인** (0.2.20 의 "엔진 미지원일 수 있다" 우려 해소).
  노트 볼트에서 `AR_시스템-지출원장` 을 앤솔로지 2건에 부여한 결과, Atlas 에 전용 노드
...
### Notes
- 노트 볼트는 **서버 맥 → 타 맥 단방향 미러**(`rsync --delete`)다. 리모트 맥의
