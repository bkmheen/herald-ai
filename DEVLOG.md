...
> **현재 묶음: 5 — 교차 검토 후 정리** (slug: `audit`, `0.2.44`~)
>
> ⓘ 2026-08-16 에 **전 이력을 소급 재번호**했다 (`0.1.0`~`0.9.202` → `0.2.10`~).
> 최초 커밋부터 지금까지가 한 체계이고 **역행 구간이 없다.**
> 같은 날 **작성자 신원까지 재작성**했다 (`0.2.48`).
...
## 2026-08-16 — 세 갈래 교차 검토와 네 번째 이력 재작성 (v0.2.44~v0.2.48)

### 왜 검토했나

재번호 작업(`0.2.43`)을 끝낸 직후, 사용자가 **"현재 상황에 문제가 없는지 세 개의 서브에이전트에
검토를 요청하고 결과를 리뷰하라"** 고 지시했다. 셋에게 서로 다른 각을 줬다 — 버전 정합성 ·
문서 정합성 · 공개 저장소 감사.

**받은 보고를 그대로 옮기지 않고 전부 재현했다.** 이것이 결정적이었다.

### 검증 도구 자체가 거짓말을 했다

감사 에이전트가 경고를 달아 왔다 — `git grep <패턴> $ALL --` 형태로 **변수를 거쳐** 리비전
목록을 넘기면 이 셸에서 git 이 조용히 **0건**을 반환한다. 나도 그대로 당했다.

```
$ ALL=$(git rev-list --all); git grep -l "확실히-있는-문자열" $ALL --
(0건)                                   ← 거짓. HEAD 에 버젓이 있다
$ git grep -l "확실히-있는-문자열" $(git rev-list --all) --
2                                       ← 참
```

**대조군을 함께 돌리지 않으면 "깨끗하다" 와 "스캔이 죽었다" 를 구별할 수 없다.**
이후 모든 전 이력 검사에 `bkmheen`(반드시 검출되는 문자열)을 같이 돌려 스캔의 생존을 확인했다.
`0.2.39` 의 "전 이력 재검사 결과 검출 없음" 도 이런 식으로 얻은 거짓 음성이었을 가능성이 높다.

### 놓쳤던 것 — 이력 재작성은 신원을 건드리지 않는다

`0.2.38` 의 재작성은 **파일 내용**(`--replace-text`)과 **커밋 메시지**(`--replace-message`)를
치환했다. 그런데 커밋 6건의 작성자 이메일이 머신 기본값(`<계정>@<호스트>.local`)이라
**개인 계정명과 실제 맥 이름이 그대로 공개돼 있었다.**

```
34  bkmheen@gmail.com            ← 의도한 공개 신원
 6  <계정>@<호스트>.local          ← 머신 기본값. origin/main 에서 도달 가능
```

작성자 신원은 **별개의 메타데이터**라 어느 치환에도 걸리지 않는다. `--mailmap` 이 따로 있다.
이름은 양쪽 다 같았고 이메일만 달랐다.

> **재작성 점검표에 신원을 넣는다.** 파일 내용 · 커밋 메시지 · **작성자/커밋터 신원** ·
> 태그 메시지 — 네 곳이다. 하나를 빼면 "다 지웠다" 는 착각만 남는다.

### 조각도 개인정보다

완전한 IP 는 저장소 어디에도 없었다(0건). 그런데 **대역 표기**와 **끝옥텟 예시**가 서로 다른
파일에 남아, 둘을 합치면 실주소가 나왔다. 단일 문자열 기준의 검사는 이것을 통과시킨다.

**"검출 0건" 은 "복원 불가" 가 아니다.**

### 원칙은 한 번 쓴다고 지켜지지 않는다

실제 맥 이름 2종이 다시 들어온 곳은 `0.2.42` 항목이다. 그 원칙을 세우고 호스트 이름 142곳을
걷어낸 것이 **바로 앞 묶음(`0.2.38`~`0.2.39`)** 이었다. 규칙을 문서에 쓰는 일과 그 규칙대로
글을 쓰는 일은 별개이고, 후자는 매번 다시 해야 한다.

**나도 즉시 같은 실수를 했다.** 바로 위 「검증 도구가 거짓말을 했다」의 예시를 쓰면서
**실제 맥 이름을 검색어로 그대로 적었다.** 재작성으로 그 이름을 지운 직후에, 그것을 지웠다고
설명하는 문장에서 되살린 것이다 (`0.2.49` 에서 자리표시자로 교체).

> **예시에 실제 값을 쓰지 않는다.** 값이 무엇인지는 설명에 필요하지 않다.
> 커밋 직전 검사가 이것을 잡았다 — 그래서 검사는 커밋 **전에** 돌린다.

### 규약을 쓴 것과 적용한 것 사이의 간격

문서 검토가 짚은 것 중 무거운 쪽은 오탈자가 아니라 **규약 준수 상태**였다.

| 규약 | 실제 |
|---|---|
| `x0` 커밋에 묶음 머리말 | 넷 중 셋이 없고, `x0` 아닌 곳에 붙어 있었다 |
| zero-padding 없음 | `묶음 02` (폐기된 `묶음×100` 체계의 흔적) |
| 태그는 주제 경계에 | `numbering` 이 전 이력을 감싸 다른 셋을 삼켰다 |
| `VERSION` 이 단일 출처 | README 가 23판 뒤처졌다 (**두 번째 재발**) |

README 는 값을 옮겨 적는 대신 **`VERSION` 을 가리키게** 바꿨다. 두 곳에 적으면 반드시 한쪽이
낡는데, 실제로 두 번 낡았다. 같은 이유로 트레일러 규정의 모델명도 고정값을 뺐다.

### 남긴 것

- 옛 규약의 **예시 숫자**(`묶음×100` 공식 설명문에 쓰인 값)가 과거 커밋에 남아 있다.
  실제로 부여된 적 없는 번호이고, **그 체계를 설명하던 대목에서는 맞는 예시**다.
  자리표시자로 바꾸면 역사 기록이 오히려 부정확해지므로 그대로 둔다.
- 개인 볼트·노트 도구의 디렉토리 이름은 `trip-ledger` 스킬이 **실제로 호출하는 경로**라
  문서상 필요한 계약이다. 추상화(`$VAULT_ROOT` 류)는 별도 과제로 남긴다.
...
- **`0.1` 구간은 비워 두고 다음 구간 `0.2` 를 할당**했다 (사용자 지시).
  `0.1` 부터 다시 매기면 새 번호가 옛 대화에 남은 `0.1.x` 와 같은 값을 가리켜 어느 쪽인지
  분간할 수 없다. 아예 쓰지 않은 구간으로 옮겨 그 충돌을 피한 것이다
  (참고: `v0.1.x` 태그는 애초에 만들어진 적이 없어 지울 것도 없었다)
...
- **이력 재작성** — 아직 clone 한 사람이 없어 지금이 마지막 기회라는 사용자 판단.
  ...
  두 번에 나눠 돌았다 — 1차는 정확한 주소만 잡아 **대역 표기**(앞 세 옥텟만 적은 형태)를 놓쳤다.
  재작성 전 번들 백업을 남겼다
...
- 서버 맥: 볼트 상위가 **실디렉토리**(정본). 리모트 맥: **심볼릭 링크**(수신본).
- 볼트의 미러 스크립트가 `--delete` 로 정본 → iCloud 사본을 rsync.
...
3. **스캔 자체가 느림** — 캐시를 데운 뒤 `--offline` 으로 재실행해도 **33초**. 즉 npm 이
   아니라 `ccusage` 가 전체 트랜스크립트(이 머신에 누적된 수천만 토큰)를 매번 스캔하는 게 병목.
...
- `stop`: `월누적` 이 `$0.0` → 실제 누적액으로 정상 표시. 59일 인자는 첫 호출이라 스캔(캐시 미스) 후 캐시.
...
session-log)과 다른 주제여서, **당시 규칙**("주제가 바뀌면 윗자리를 올린다")에 따라 번호를
올렸다 → `0.2.16`.

> ⓘ 이 규칙은 규약 v3.1.0 에서 폐기됐다. 지금은 **묶음이 주제가 아니라 10커밋 구간**이고,
> 주제는 마일스톤 태그와 이 DEVLOG 가 잇는다. `minor` 는 Claude 가 자체 판단으로 올릴 수 없다.
...
- "Atelier 제목 표준" 등 **볼트 쪽 명명 규정**을 참조한다 (규정 자체는 비공개 볼트에 있다).
- 출력 경로 `~/Desktop/` 하드코딩.
