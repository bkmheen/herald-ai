...
> **현재 묶음: 01 — 버전 체계 개정** (slug: `convention`, `0.2.37~`)

날짜별 개발 의사결정·작업 내역을 시간순으로 남깁니다.
사용자용 변경 요약은 [CHANGELOG.md](CHANGELOG.md), 코드 변경의 배경·근거는 본 문서에 기록합니다.

버전은 `patch = 묶음×100 + 커밋` 체계입니다 (규약 v2.0.0, `0.2.37~`). 주제가 바뀌면
이 줄의 묶음 번호를 올리고 새 버전을 `x00` 으로 엽니다 — 자세한 규칙은 [CLAUDE.md](CLAUDE.md).
...
옛 방식(patch 단순 증가: 0.2.23 → 0.2.28 → 0.2.36)을 계속 쓰고 있었다.
...
- 소급 개정은 하지 않는다. 이미 push 된 0.2.22~0.2.36 의 번호를 다시 쓰면 히스토리 재작성이 되고,
  규약 §11 도 "번호 재작성은 하지 않는다"는 선례를 남겼다
- 시작점은 사용자 지시대로 **`0.2.37`** — minor 를 하나 올리고(0.8→0.9) 묶음 01·커밋 00 으로 연다
