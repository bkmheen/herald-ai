...
규약 §3.3 은 커밋 트레일러 4줄(`Co-Authored-By`/`Worked-On`/`Committed-To`/`Model`)을 요구하는데,
`Worked-On` 은 **호스트 이름과 사설 IP** 를 커밋 메시지에 적게 되어 있다.
herald-ai 는 **public 저장소**이고 설계 ①§0 이 "herald-ai 는 public 이라 서버 IP 가 섞이면
되돌릴 수 없다"고 못박아 두었다. 이번 커밋에는 트레일러를 확대 적용하지 않고 질의로 남긴다.
